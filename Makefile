...
ServiceUpperName ?= "Log"
ServiceLowerName ?= "log"
...
# 复制proto文件
	...
	@cp ./api/log/v1/log.proto ./api/$(ServiceLowerName)/v1/$(ServiceLowerName).proto

# 删除旧文件
	@rm -rf ./api/log
# 生成proto客户端文件
	@ kratos proto client ./api/$(ServiceLowerName)/v1/$(ServiceLowerName).proto
# 替换 Log $(ServiceUpperName), log 为$(ServiceLowerName)并迁移文件
	@grep  -rl Log ./ | xargs sed -i 's/Log/$(ServiceUpperName)/g'
	@grep  -rl log ./ | xargs sed -i 's/log/$(ServiceLowerName)/g'

# 迁移文件
	@ mv ./internal/service/log.go ./internal/service/$(ServiceLowerName).go
	@ mv ./internal/biz/log.go ./internal/biz/$(ServiceLowerName).go
	@ mv ./internal/data/log.go ./internal/data/$(ServiceLowerName).go
	@ mv ./internal/domain/log.go ./internal/domain/$(ServiceLowerName).go
# 拉取引用包
	go mod tidy
